...
SGD_LR = ADAM_LR / 10  # Learning rate для SGD (в 10 раз меньше)
INIT_EPOCHS = 5  # Эпохи для начальной инициализации с Adam
CONVERGENCE_EPOCHS = 15  # Эпохи для проверки сходимости с SGD
PCA_COMPONENTS = 50  # Число компонент PCA для ускорения вычислений
SAVE_DIR = "data/checkpoints/adam_sgd_valley_test"
...
def main():
    ...
    # Оптимизаторы Adam для начального обучения и функция потерь
    global criterion
    criterion = nn.CrossEntropyLoss()
    optimizer1_adam = torch.optim.Adam(model1.parameters(), lr=ADAM_LR)
    optimizer2_adam = torch.optim.Adam(model2.parameters(), lr=ADAM_LR)
    
    # Начальное обучение с Adam (5 эпох)
    print("Начальное обучение моделей с Adam (5 эпох)...")
    trajectory1_init, losses1_init = train_and_collect_trajectory(model1, optimizer1_adam, INIT_EPOCHS, "Model1 (Adam)")
    trajectory2_init, losses2_init = train_and_collect_trajectory(model2, optimizer2_adam, INIT_EPOCHS, "Model2 (Adam)")
    ...
    # Создаем оптимизаторы SGD для проверки сходимости
    ...
    print("\nПроверка сходимости моделей с SGD...")
    trajectory1_conv, losses1_conv = train_and_collect_trajectory(model1, optimizer1_sgd, CONVERGENCE_EPOCHS, "Model1 (SGD)")
    trajectory2_conv, losses2_conv = train_and_collect_trajectory(model2, optimizer2_sgd, CONVERGENCE_EPOCHS, "Model2 (SGD)")
    ...
    # Рисуем фазу инициализации с Adam
    plt.plot(init_steps, losses1[:len(losses1_init)], 'b-', label='Model 1 (Adam)')
    plt.plot(init_steps, losses2[:len(losses2_init)], 'r-', label='Model 2 (Adam)')
    
    # Рисуем фазу сходимости с SGD
    plt.plot(conv_steps, losses1[len(losses1_init):], 'b--', label='Model 1 (SGD)')
    plt.plot(conv_steps, losses2[len(losses2_init):], 'r--', label='Model 2 (SGD)')
    
    # Добавляем заливку для фазы сходимости
    plt.axvspan(len(losses1_init), len(losses1), alpha=0.2, color='gray', label='Фаза сходимости (SGD)')
    
    # Добавляем вертикальную линию разделения
    plt.axvline(x=len(losses1_init), color='r', linestyle='--', label='Переход Adam → SGD')
    
    plt.xlabel('Шаги')
    plt.ylabel('Потери')
    plt.title('Кривые потерь для двух моделей (Adam → SGD)')
    plt.legend()
    plt.grid(True)
    ...
    plt.title('Расстояние между параметрами моделей в фазе SGD')
    plt.grid(True)
    plt.savefig(os.path.join(SAVE_DIR, "parameter_distance.png"))
    plt.close()
    
    # Сохраняем результаты
    np.save(os.path.join(SAVE_DIR, "trajectory1_adam.npy"), trajectory1_init)
    np.save(os.path.join(SAVE_DIR, "trajectory2_adam.npy"), trajectory2_init)
    np.save(os.path.join(SAVE_DIR, "trajectory1_sgd.npy"), trajectory1_conv)
    np.save(os.path.join(SAVE_DIR, "trajectory2_sgd.npy"), trajectory2_conv)
