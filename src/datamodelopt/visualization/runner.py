...
class ManyRunsVisualizer:
    ...
    def plot_hessian_spectrum(
        ...
        Args:
            n_left: Number of leftmost eigenvalues to show.
            n_right: Number of rightmost eigenvalues to show.
            eps: Threshold for near-zero eigenvalues.
        """
        from .plots import plot_hessian_spectrum

        plot_hessian_spectrum(
            run_dirs=[str(d) for d in self.run_dirs],
            stages=self.stages,
            save_path=str(self.output_dir / "hessian_spectrum.png"),
            n_left=n_left,
            n_right=n_right,
            eps=eps,
            title=f"Hessian Eigenvalue Spectrum ({len(self.run_dirs)} runs)",
        )
