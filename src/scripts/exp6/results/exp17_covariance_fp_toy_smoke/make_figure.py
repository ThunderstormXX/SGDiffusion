...
def make_exp17_covariance(rows: list[dict[str, str]], out: Path, title: str, setup: str) -> None:
    fig, axes = plt.subplots(
        ...
        3,
        figsize=(14.5, 5.2),
        gridspec_kw={"width_ratios": [1.1, 1.45, 1.45]},
    )
    draw_setup_box(axes[0], setup)
    label_map = {
    ...
    for method in methods:
        ...
        axes[1].loglog(x_plot, np.maximum(y, 1e-18), label=label_map.get(method, method))
    axes[1].set_xlabel("iteration")
    axes[1].set_ylabel("trace covariance")
    axes[1].set_title("covariance trace (log-log)")
    axes[1].grid(alpha=0.25)
    axes[1].legend(fontsize=8)
    ...
    for method in ["standard_fp", "discrete_fp"]:
        ...
        axes[2].loglog(x_plot, np.maximum(y, 1e-18), label=label_map.get(method, method))
    axes[2].set_xlabel("iteration")
    axes[2].set_ylabel("relative Frobenius error")
    axes[2].set_title("error to empirical covariance")
...
def make_exp27_phase(result_dir: Path, rows: list[dict[str, str]], out: Path, title: str, setup: str) -> None:
    ...
    fig, axes = plt.subplots(1, 3, figsize=(15.0, 5.2), gridspec_kw={"width_ratios": [1.05, 1.35, 1.35]})
    draw_setup_box(axes[0], setup)
    im = axes[1].imshow(empirical, origin="lower", aspect="auto", extent=[etas.min(), etas.max(), gammas.min(), gammas.max()], cmap="Reds")
    axes[1].set_xlabel(r"$\eta$")
    axes[1].set_ylabel(r"$\Gamma$")
    axes[1].set_title("empirical unstable region")
    plt.colorbar(im, ax=axes[1], fraction=0.046)
    eta_grid = np.linspace(etas.min(), etas.max(), 200)
    lam = _load_optional_json(result_dir / "metrics.json").get("lambda", 1.0)
    axes[1].plot(eta_grid, lam * (2 - eta_grid * lam) / eta_grid, "k-", label="discrete boundary")
    axes[1].plot(eta_grid, 2 * lam / eta_grid, "k--", label="standard boundary")
    axes[1].set_ylim(gammas.min(), gammas.max())
    axes[1].legend(fontsize=8)
    if "example_variance_trace" in data:
        trace = data["example_variance_trace"]
        axes[2].semilogy(np.arange(len(trace)), np.maximum(trace, 1e-18))
        axes[2].set_title(f"mismatch example eta={float(data['example_eta']):.3g}, Gamma={float(data['example_gamma']):.3g}")
    else:
        axes[2].text(0.5, 0.5, "No mismatch example", ha="center", va="center")
...
def main() -> None:
    ...
    elif result_dir.name.startswith("exp17_") and "covariance_trace" in rows[0]:
