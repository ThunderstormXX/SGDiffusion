...
def load_weights_and_evaluate(weights_file, val_loader, device, n_samples=100):
    ...
    model_template = get_model_for_dataset('shakespeare', device=device)
...
def main():
    ...
    # Load dataset
    dataset_name = 'shakespeare'  # For now, hardcode to shakespeare as this script is specific to it
    _, _, _, val_loader = load_dataset(dataset_name, batch_size=32)
    ...
    for weights_file in weights_files:
        ...
        # Evaluate weights
        results = load_weights_and_evaluate(weights_file, val_loader, device, args.n_samples)
