...
from data_utils import load_gd_dataset, get_model_for_dataset
...
def main():
    ...
    # Load dataset based on the specified name (with full batch for training)
    train_ds, val_ds, train_loader_full, val_loader = load_gd_dataset(
        args.dataset_train, batch_size=32, seed=args.seed, sample_size=args.sample_size  # val batch size
    )
    ...
    weight_traj_path = os.path.join(args.results_dir, "gd_weight_trajectory.npy")
    np.save(weight_traj_path, np.array(weight_trajectory))
    print(f"[saved] {weight_traj_path}")
